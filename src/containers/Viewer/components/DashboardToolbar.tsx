...
const DashboardToolbar = ({ onDatasetChange, dashboardService }) => {
  const [dataset, setDataset] = React.useState(dashboardService.dataset);
  ...
  const checkDataset = async () => {
    if (dataset) {
      ...
      const isReachable =
        await dashboardService.dataService.checkDataset(dataset);
      if (isReachable) {
        setColor("success");
  ...
  function linkDataset() {
    const url = prompt(
      "Please enter SPARQL endpoint URL of the dataset to link and leave empty to unlink:"
    );
    if (url) {
      setDataset(url);
  ...
  return (
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="static">
        <Toolbar>
          <IconButton color="inherit" edge="start" sx={{ mr: 2 }}></IconButton>
          <Typography
            variant="h6"
            component="div"
          ...
          <Box sx={{ display: { xs: "none", md: "flex" } }}>
            ...
            >
              ...
              <VisuallyHiddenInput
                type="file"
                onChange={(event) =>
                  dashboardService.importDashboard(event.target.files)
                }
              />
            </Button>
          </Box>
